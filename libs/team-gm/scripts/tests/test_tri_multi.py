...
from team_gm.modules.attentions import TriangleMultiplication

SHAPES = [32, 64, 128, 256, 512]

OUT_GOING = [True, False]


@pytest.fixture(params=[(M, out_going) for M in SHAPES for out_going in OUT_GOING])
def setup(request):
    M, outgoing = request.param
    N = 128
    B = 5
    device = torch.device("cuda")
    suffix = f"[{M}, {M}, {N}], {outgoing}"

    rtol = 5e-2
    atol = 5e-2

    torch_model = TriangleMultiplication(
        implementation="pytorch",
        outgoing=outgoing,
    ).to(device)
    triton_model = TriangleMultiplication(
    ...
    class TestModule(nn.Module):
        def __init__(self, k=1, implementation="pytorch"):
            ...
            self.layers = nn.ModuleList(
                [
                    TriangleMultiplication(implementation=implementation, outgoing=True)
                    for _ in range(k)
                ]
            )
