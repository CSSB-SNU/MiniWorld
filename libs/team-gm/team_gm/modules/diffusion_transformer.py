...
from team_gm.data.atom_layout import convert, GatherInfo


def mask_mean(mask, value, axis=None, keepdims=False, eps=1e-9):
    ...
    assert isinstance(axis, abc.Iterable), (
        "axis must be an integer or a iterable of integers, got {}".format(type(axis))
    )
    ...
    for axis_ in axis:
        ...
        if mask_size == 1:
            ...
            assert mask_size == value_size, (
                "Shapes are not compatible, shapes: {}, {}".format(
                    mask_shape, value_shape
                )
            )
...
class DiffusionTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conditioned_transition = ConditionedTransition(
            ...
            implementation=implementation,
            use_checkpoint=False if self.use_checkpoint else True,
        )
    ...
    def _forward(
        ...
        atom_single_rep = atom_single_rep + self.conditioned_transition(
            atom_single_rep,
            atom_single_cond,
        )
...
class AtomAttentionEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.token_single_to_atom_single_cond = nn.Sequential(
            ...
            Linear(
                common_config.d_token_single, d_atom_single, bias=False, init="final"
            ),
        )
        self.token_pair_to_atom_pair = nn.Sequential(
            LayerNorm(d_token_pair, implementation=diffusion_config.implementation),
            # LayerNorm(d_token_pair, implementation="pytorch"),
            Linear(d_token_pair, d_atom_pair, bias=False, init="final"),
        )
        self.noisy_to_atom_single_rep = Linear(3, d_atom_single, bias=False)
        ...
        self.mlp_atom_pair = nn.Sequential(
            ...
            Linear(d_atom_pair, d_atom_pair, init="relu", bias=False),
            nn.ReLU(),
            Linear(d_atom_pair, d_atom_pair, init="final", bias=False),
        )
    ...
    def _add_trunk_info(
    ...
    ) -> tuple[Float[torch.Tensor, "B L d_atom_single_rep"],
        ...
        if not self.use_beta:
            ...
            atom_single_rep = atom_single_rep + self.noisy_to_atom_single_rep(
            ...
            queries_single_cond = convert(
                noisy_batch.cross_att.token_atoms_to_queries, # TODO
                atom_single_cond,
                layout_axes=(-3, -2),
            )
            queries_single_cond = queries_single_cond + convert(
                noisy_batch.cross_att.tokens_to_queries,
                _to_add_single,
                layout_axes=(-2,),
            )

            queries_single_rep = convert(
                noisy_batch.cross_att.token_atoms_to_queries,
                noisy_batch.x_t.to(torch.float32),
                layout_axes=(-3, -2),
            )
            queries_single_mask = convert(
                noisy_batch.cross_att.token_atoms_to_queries,
                noisy_batch.atom_mask,
                layout_axes=(-2, -1),
            )
            queries_single_rep = self.noisy_to_atom_single_rep(queries_single_rep)
            queries_single_rep = (
                queries_single_rep * queries_single_mask[None, ..., None]
            )
            queries_single_rep = (
                queries_single_rep + queries_single_cond
            )  # (A,B,num_subset,Q,d_atom_single)

            # Create the GatherInfo into a flattened trunk_pair_cond from the
            # queries and keys gather infos.
            num_tokens = _to_add_pair.shape[1]
            # (num_subsets, num_queries)
            tokens_to_queries = noisy_batch.cross_att.tokens_to_queries
            # (num_subsets, num_keys)
            tokens_to_keys = noisy_batch.cross_att.tokens_to_keys
            ...
            trunk_pair_to_atom_pair = GatherInfo(
                ...
                input_shape=np.array((num_tokens, num_tokens)),
            )
            # Gather the conditioning and add it to the atom-pair activations.
            atom_pair = atom_pair + convert(
                trunk_pair_to_atom_pair, _to_add_pair, layout_axes=(-3, -2)
            )

            _left = self.atom_single_to_pair_left(queries_single_cond)
            keys_single_cond = convert(
                noisy_batch.cross_att.queries_to_keys,
                queries_single_cond,
                layout_axes=(-3, -2),
            )
            _right = self.atom_single_to_pair_right(keys_single_cond)
    ...
    def _get_input_feature(
        self, 
        noisy_batch: NoisyBatch
    ) -> tuple[Float[torch.Tensor, "B L d_atom_single_cond"],
        ...
        ref_infos = torch.cat(
            [
                ...
                torch.arcsinh(noisy_batch.ref_charge).unsqueeze(-1),
            ],
            dim=-1,
        )
        ...
        if not self.use_beta:
            d_lm = noisy_batch.reference.pos[:, :, None] - noisy_batch.reference.pos[:, None, :]
            v_lm = (
            ...
            atom_ref_pos = noisy_batch.reference.pos  # (num_tokens, max_atoms_token, 3)
            ref_space_uid = noisy_batch.reference.space_uid  # (num_tokens, max_atoms_token)

            # Embed offsets between atom reference positions
            queries_ref_pos = convert(
                noisy_batch.cross_att.token_atoms_to_queries,
                atom_ref_pos,
                layout_axes=(-3, -2),
            )
            queries_ref_space_uid = convert(
                noisy_batch.cross_att.token_atoms_to_queries,
                ref_space_uid,
                layout_axes=(-2, -1),
            )

            keys_ref_pos = convert(
                noisy_batch.cross_att.queries_to_keys,
                queries_ref_pos,
                layout_axes=(-3, -2),
            )
            keys_ref_space_uid = convert(
                noisy_batch.cross_att.queries_to_keys,
                queries_ref_space_uid,
                layout_axes=(-2, -1),
            )

            d_lm = queries_ref_pos[..., None, :] - keys_ref_pos[..., None, :, :]
            v_lm = queries_ref_space_uid[..., None] == keys_ref_space_uid[..., None, :]
        v_lm = v_lm[..., None]
    ...
    def _scatter_atom_to_token(
        ...
        if not self.use_beta:
            atom_to_residue_idx_map = noisy_batch.scheme.atom_to_residue_idx_map  # (B, L_atom)
            atom_mask = noisy_batch.atom_mask  # (B, L_atom)
            ...
            atom_single_rep = convert(
                noisy_batch.cross_att.queries_to_token_atoms,
                to_add_token_single_rep,
                layout_axes=(-3, -2),
            )

            token_single_rep = mask_mean(atom_mask[..., None], atom_single_rep, axis=-2)
    ...
    def forward(
        ...
        token_single_cond: Float[torch.Tensor, "B L_token d_token_single"],
        token_pair_cond: Float[torch.Tensor, "B L_token L_token d_token_pair"],
    ) -> tuple[Float[torch.Tensor, "B L_token d_token_single_rep"],
...
class AtomAttentionDecoder(nn.Module):
    def __init__(
        ...
        self.final_denoising = nn.Sequential(
            LayerNorm(d_atom_single, implementation="pytorch"),
    ...
    def forward(
        ...
        x_denoised = self.final_denoising(atom_single_rep)
        return x_denoised  # (A, B, L_atom, 3)
