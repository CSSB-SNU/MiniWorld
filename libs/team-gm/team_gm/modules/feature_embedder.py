...
from .diffusion_transformer import DiffusionTransformer, mask_mean
...
def FourierEmbedding(sigma_nosie_level: torch.Tensor) -> torch.Tensor:
...
class InputAtomAttentionEncoder(nn.Module):
    ...
    def _get_input_feature(
    ...
    ) -> tuple[Float[torch.Tensor, "B L d_atom_single"], Float[torch.Tensor, "B L L d_atom_pair"]]:
        """
        ...
        with torch.no_grad():
            if not self.use_beta:
                ...
                ref_space_uid = noisy_batch.reference.space_uid
    ...
    def _scatter_atom_to_token(
        self, noisy_batch: NoisyBatch, atom_single_rep: Float[torch.Tensor, "B L d_atom_single"]
    ...
    def forward(
        ...
        if self.use_checkpoint:
             atom_single_rep, atom_single_cond, atom_pair = torch.utils.checkpoint.checkpoint(
        ...
        if self.use_checkpoint:
            token_single_rep = torch.utils.checkpoint.checkpoint(
                self._scatter_atom_to_token, noisy_batch, atom_single_rep, use_reentrant=False
            )
        else:
            token_single_rep = self._scatter_atom_to_token(
                noisy_batch, atom_single_rep
            )
...
class RelativePositionEmbedding(nn.Module):
    ...
    def forward(self, noisy_batch: NoisyBatch) -> Float[torch.Tensor, "B L L d_hidden"]:
        with torch.no_grad():
            ...
            # b_same_residue = residue_idx[:, :, None] == residue_idx[:, None, :]
        ...
        token_pair = self.embed_rel_pos(token_pair)  # (B, L, L, d_hidden)
        return token_pair  # (..., L, L, d_hidden)
...
class InputFeatureEmbedder(nn.Module):
    ...
    @torch.no_grad()
    def _gen_bond_feature(self, noisy_batch: NoisyBatch) -> Float[torch.Tensor, "B L_token L_token 2"]:
        #  -> tuple[torch.Tensor, torch.Tensor]:
        B = noisy_batch.shape[0]
        L_token = noisy_batch.residue_length
        residue_bond = noisy_batch.structure.residue_bond  # (B, n_residue_bond, 3)
    ...
    def forward(
        self, 
        noisy_batch: NoisyBatch
    ) -> tuple[Float[torch.Tensor, "B L_token d_token_single_input"],
