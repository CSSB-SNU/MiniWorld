...
class DtypeConfig(BaseModel):
    ...
    op_dtype: torch.dtype = Field(
        default=torch.float32,
        metadata={"help": "Data type for operations"},
    )
    out_dtype: torch.dtype = Field(
        default=torch.float32,
...
def _wrap_with_casts(module_cls, op_dtype, out_dtype):
    class Wrapped(module_cls):  # type: ignore
        def forward(self, *args, **kwargs):
            ...
            args = cast_data(args, op_dtype)
            kwargs = cast_data(kwargs, op_dtype)

            if op_dtype != torch.float32:
                with autocast(device_type=device_type, enabled=True, dtype=op_dtype):
                    out = super().forward(*args, **kwargs)
            else:
                out = super().forward(*args, **kwargs)

            out = cast_data(out, out_dtype)  # ensure output is in out_dtype
            return out
