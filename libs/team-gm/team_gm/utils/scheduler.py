...
class DiffusionScheduler(ABC):
    ...
    @abstractmethod
    def sample_noise(self, B: int) -> torch.Tensor:
...
class EDM_Scheduler(DiffusionScheduler):
    """
    ...
    class EDM_SchedulerConfig(DiffusionScheduler.DiffusionSchedulerConfig):
        method: str = "AF3"
        P_mean: float = -1.2
        P_std: float = 1.5
    ...
    def __init__(self, config: EDM_SchedulerConfig):
        self.config = config
    ...
    def sample_noise(self, B: int) -> torch.Tensor:
        ...
        sigma = torch.clamp(sigma, min=self.config.sigma_min * self.config.sigma_data, max=self.config.sigma_max*self.config.sigma_data)
        return sigma  # (B,)
