...
def resolver(monkeypatch):
    ...
    def publish(config, *, identity='implementation'):
        ...
        row['ms'] = 1.0
        state['data'] = dict(build_rev=1, op_identity=identity, env_identity='compiler',
                             key_scheme=cache.KEY_SCHEME,
                             entries={'bfloat16|shape': [row]})

    def resolve(candidates=None):
        return cute_config.resolve_config('trimul_inproj_masked_sm90_cute',
                                          space if candidates is None else candidates,
                                          dtype='bfloat16', bucket='shape')
