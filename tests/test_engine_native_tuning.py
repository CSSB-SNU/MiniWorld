...
from miniworld_engine.autotune import cache, capture, cute_config, native, native_compile
from miniworld_engine.autotune import native_history
...
def tuner(tmp_path, monkeypatch):
    ...
    def precompile(op, configs, bucket):
        ...
        return {i: state['results'][c['tile']] for i, c in enumerate(configs)
                if c['tile'] in state['results']}
    monkeypatch.setattr(native_compile, 'precompile', precompile)
    def run(c):
        state['ran'].append(c['tile'])
        state['last'] = c['tile']
    monkeypatch.setattr(triton.testing, 'do_bench', lambda *a, **kw: float(state['last']))
    def choose(tiles, bucket='physical shape A'):
        native.reset()
        return native.choose_config(OP, [{'tile': n} for n in tiles], dtype='bfloat16',
                                    bucket=bucket, run=run)
...
def test_checkpoint_survives_interruption(tmp_path):
    with pytest.raises(KeyboardInterrupt):
        with native_history.session(str(tmp_path), ('identity',)) as history:
            history.record('one', {'status': 'ok', 'ms': .1})
            raise KeyboardInterrupt
    with native_history.session(str(tmp_path), ('identity',)) as history:
        assert history.records['one']['ms'] == .1
...
@pytest.mark.parametrize('factory,count', [
    ('gated_sm90_candidates',448), ('plain_sm90_candidates',512),
    ('fused_lnl_candidates',240), ('lnbwd_pp_candidates',48), ('tm2_candidates',4),
])
...
def test_fixed_epilogue_constraints():
    assert all(not c.is_dynamic_persistent and c.tile_m != 192
    ...
    assert all(c.cluster_n == 1 and c.pingpong for c in cute_config.lnbwd_pp_candidates())
...
def test_swizzle_compile_dedup_keeps_scheduler_distinct():
    ...
    bucket = repr(((((264,128),(128,1),'torch.bfloat16'),
                    ...
                    ((1,264),(264,1),'torch.float32')), (True,)))
    task = lambda cfg: native_compile.task_for(OP, cute_config.config_to_kwargs(cfg), bucket)
    assert native_compile.task_id(task(c)) == native_compile.task_id(task(replace(c,max_swizzle_size=1)))
    assert native_compile.task_id(task(c)) != native_compile.task_id(task(replace(c,is_dynamic_persistent=True)))
...
def test_policy_edit_does_not_invalidate_source_identity(monkeypatch):
    ...
    def changed(path, *a, **kw):
        ...
        return text.replace('SWIZZLES = (1, 2, 4, 8)', 'SWIZZLES = (1, 8)') if path.name == 'cute_config.py' else text
...
def test_failure_provenance_and_rebuild_invalidates_old_winner(tuner, tmp_path):
    ...
    assert native_history.compile_failure_status({'status':'failed','returncode':-9}) == 'retryable_failure'
...
def test_policy_change_reopens_units_without_dropping_measurements(monkeypatch):
    ...
    def changed(path,*a,**kw):
        ...
        return data+b'\n# larger candidate policy\n' if path.name=='cute_config.py' else data
...
def test_runtime_uses_explicit_last_native_profile(tuner, monkeypatch):
    ...
    capture.reset()
    settings.configure(bench_rep_ms=2)
    monkeypatch.setattr(triton.testing,'do_bench',lambda *a,**kw:3.-t['last'])
    assert t['choose']([1,2])=={'tile':2}
    capture.flush(gpu='test_h100')
    selected=cache.select_config(OP,dtype='bfloat16',bucket='physical shape A',
        candidates=[cache.as_cfg_dict({'kwargs':{'tile':i}}) for i in (1,2)],op_id=native.source_identity())
