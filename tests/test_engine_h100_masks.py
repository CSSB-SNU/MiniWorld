...
def models(kind):
    ...
    actual = (
        cls(128, p_drop=0, implementation=ImplementationType.MINIWORLD, **options)
        .cuda()
        .to(torch.bfloat16)
    )
    with torch.no_grad():
...
def test_training_fused_mask_output_and_all_gradients(kind, length, mask_kind):
    ...
    mask = (
        None
        if mask_kind == "none"
        else torch.ones(1, length, device="cuda", dtype=torch.bool)
    )
    if mask_kind == "holes":
    ...
    for (name, p), (rn, rp) in zip(
        actual.named_parameters(), ref.named_parameters(), strict=True
    ):
        assert name == rn
...
def test_inference_fused_mask_preserves_reference(kind, length, mask_kind):
    ...
    if mask_kind == "holes":
        ...
        mask.zero_()
    with torch.no_grad():
        y, yr = actual(x, mask), ref(x.float(), mask)
    check("output", y, yr)
    if mask_kind == "all_invalid":
        torch.testing.assert_close(y, x, rtol=0, atol=0)
...
def test_hopper_mask_keeps_output_gate_and_custom_eps(kind, training):
    ...
    with torch.no_grad():
        for model in (actual, ref):
            model.ln_pair.eps = 0.03
            model.ln_out.eps = 0.07
        actual.ln_pair.bias.fill_(0.2)
        actual.ln_out.bias.fill_(0.4)
    ref.load_state_dict(actual.state_dict())
    x = torch.randn(
        1, 128, 128, 128, device="cuda", dtype=torch.bfloat16, requires_grad=training
    )
