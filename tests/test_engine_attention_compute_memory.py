...
from miniworld_engine.kernels.augmented_attention.reference import augmented_attention_pair_bias_pytorch


@pytest.mark.parametrize('training',[True,False])
def test_large_shape_keeps_compute_efficient(monkeypatch,training):
    monkeypatch.setattr(interface,'_pair_bias_compute_efficient',lambda *args:'compute')
    monkeypatch.setattr(interface,'_pair_bias_memory_efficient',lambda *args:'atomic')
    ...
    with torch.set_grad_enabled(training):
        ...
        assert interface.triton_augmented_attention_pair_bias(q,q,q,b,compute_efficient=False)=='atomic'
...
def pin(monkeypatch,bm=32,bn=64,stages=2):
    ...
    for fn,kwargs in [(main._attn_fwd,{'BLOCK_M1':32,'BLOCK_M2':64}),
                      (main._attn_bwd_preprocess,{'BLOCK_M1':32}),
                      (main._attn_bwd,{'BLOCK_M1':bm,'BLOCK_M2':bn}),
                      (main._dq_reduce,{'BLOCK_E':256})]:
        monkeypatch.setattr(fn,'configs',[triton.Config(kwargs,num_warps=4,num_stages=stages)])
        fn.cache.clear()


def inputs(dtype,d=32,length=259):
    torch.manual_seed(7)
    args=[torch.randn(3,2,length,4,d,device='cuda',dtype=dtype).requires_grad_() for _ in range(3)]
    args.append(torch.randn(2,length,length,4,device='cuda',dtype=dtype).requires_grad_())
    mask=torch.rand(3,2,length,device='cuda')>.25
    mask[0,0]=False
    return args,mask
...
def test_chunked_matches_legacy_and_reference(monkeypatch,dtype,head_dim,tiles):
    ...
    monkeypatch.setattr(interface,'_pair_bias_memory_efficient',lambda *a:pytest.fail('atomic fallback'))
    ...
    for result,want in [(actual,ref),*zip(actual_grads,ref_grads)]:
        ...
        assert (result.float()-want.float()).norm()/want.float().norm()<.02
...
def test_head48_stage1_gradient(monkeypatch,length,dtype,chunked):
    ...
    args=[torch.randn(2,2,length,16,48,device='cuda',dtype=dtype).requires_grad_() for _ in range(3)]
    args.append(torch.randn(2,length,length,16,device='cuda',dtype=dtype).requires_grad_())
    mask=torch.rand(2,2,length,device='cuda')>.25
    mask[0,0]=False
    grad=torch.randn_like(args[0])
    old=torch.backends.cuda.matmul.allow_tf32
    torch.backends.cuda.matmul.allow_tf32=False
...
def test_compile_cuda_graph_and_repeatability(monkeypatch,dtype,length):
    ...
    with torch.cuda.stream(stream):
        ...
        fn=torch.compile(interface.triton_augmented_attention_pair_bias,fullgraph=True,dynamic=False)
        def step():
            for t in args:t.grad=None
            out=fn(*args,mask);out.sum().backward()
            return out,tuple(t.grad for t in args)
        out,grads=step();expected=(out.detach().clone(),tuple(g.detach().clone() for g in grads));del out,grads
        for _ in range(3):step()
    torch.cuda.current_stream().wait_stream(stream)
    graph=torch.cuda.CUDAGraph()
    with torch.cuda.graph(graph,stream=stream):captured=step()
    for _ in range(3):
        graph.replay();torch.cuda.synchronize()
        for result,want in [(captured[0],expected[0]),*zip(captured[1],expected[1])]:
            assert torch.isfinite(result).all()
