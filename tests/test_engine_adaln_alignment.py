...
def run_case(m,n,k,dtype,config,offset=0):
    ...
    sw,bw=rand(k,n)/k**.5,rand(k,n)/k**.5
    sb=rand(1,n).flatten()
    r=torch.rsqrt(x.float().var(-1,unbiased=False)+.03)
    c1=x.float().mean(-1)*r
    ...
    try:ref=(c.float()@sw.float()+sb.float()).sigmoid()*(x.float()*r[:,None]-c1[:,None])+c.float()@bw.float()
    finally:torch.backends.cuda.matmul.allow_tf32=old
    error=(y.float()-ref).norm()/ref.norm()
    assert torch.isfinite(y).all()
    assert error < (.014 if dtype==torch.bfloat16 else .002),float(error)
    return compiled
...
@pytest.mark.parametrize('config',[
    ...
    for bm,bn,bk,warps,stages,group in [
        (64,64,32,4,2,1),(128,256,16,4,1,2),(128,256,64,4,2,4),
        (128,256,128,4,3,8),(128,256,32,8,1,16),
        (256,128,32,4,1,1),(256,256,32,8,2,16),
        (64,256,32,4,1,1),(128,128,32,4,1,1),
    ]])
