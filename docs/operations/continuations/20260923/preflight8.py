...
def diff(a,b,path=''):
 ...
 return out
changes=diff(a,b);assert {c['key'] for c in changes}=={'train.engine_backend','train.grad_accum_steps'},changes
source=Path(meta['checkpoint']) if '--prepare' in sys.argv else R/'resume-8gpu.pt'
...
if '--prepare' not in sys.argv:assert torch.cuda.device_count()==8
