...
기준은 MiniWorld cu128 환경에 **실제로 설치된 miniworld_engine 소스**다. 기존 `TRIMUL_FORWARD.svg`는 학습 forward 그림이며, 이 추론 그림과 구분한다. H100, BF16, B=1, d_pair=d_hidden=128, mask 전달, 기본 layout을 그렸다. 그림은 소스 배선을 나타낸다. 아래 Triton contraction 검증은 별도로 수행했으며, 전체 모듈의 GPU 커널 프로파일은 아니다.
