...
class DistogramHead(nn.Module):
    ...
    @typecheck
    def _symmetrize(self, x: Float[torch.Tensor, "B L L C"]) -> Float[torch.Tensor, "B L L C"]:
        return (x + x.transpose(-2, -3)) / 2

    @typecheck
    def forward(self, pair: Float[torch.Tensor, "B L L C"]) -> Float[torch.Tensor, "B L L D"]:
        """Forward pass."""
