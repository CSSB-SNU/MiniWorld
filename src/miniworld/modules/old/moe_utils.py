...
def route(router_weight: torch.Tensor, x: torch.Tensor, k: int) -> tuple[torch.Tensor, torch.Tensor]:
    """Route inputs to top-k experts based on router weights."""
    # (E, N) @ (M, N) -> (M, E)
...
def loss_free_route(expert_frequency: torch.Tensor, router_weight: torch.Tensor, x: torch.Tensor, k: int) -> tuple[torch.Tensor, torch.Tensor]:
    """Route inputs to top-k experts with loss-free routing."""
    # (E, N) @ (M, N) -> (M, E)
    # Option 1.
...
def group_by_expert(
    ...
    """Group rows by expert for Mixture-of-Experts processing.
    ...
    membership.scatter_(1, topk_indices, src=torch.ones_like(topk_score, dtype=torch.bool))
...
def scatter_expert(
    ...
    """Scatter pooled rows back to (L,) by idx_map.

    Duplicates are summed. Invalid entries (=-1) ignored.
    """
    valid = idx_map >= 0
    return torch.zeros(shape, device=y.device, dtype=y.dtype).index_add_(
...
def stack_expert(
    ...
    if len(shape) not in (2, 3):
        msg = f"shape must be (L,E) or (L,E,D), got {tuple(shape)}"
    ...
    if D is not None and vals.size(-1) != D:
        msg = f"D mismatch: y has {vals.size(-1)} vs target {D}"
        raise ValueError(msg)
    if vals.size(-1) != 1:
        msg = f"For (L,E) output, y must be (N,) or (N,1), got last-dim {vals.size(-1)}"
        raise ValueError(msg)
