...
class TritonTM1Function(torch.autograd.Function):
    ...
    def forward(
        ...
        _left_weight = left_weight.to(op_dtype).contiguous()
        _left_gate_weight = left_gate_weight.to(op_dtype).contiguous()
        _right_weight = right_weight.to(op_dtype).contiguous()
        _right_gate_weight = right_gate_weight.to(op_dtype).contiguous()
        ...
        fused_sigmoid_gate_fwd_kernel[grid](
            x, _left_gate_weight, _left_weight, _right_gate_weight, _right_weight,
            left, right,
            M, N,
    ...
    def backward(ctx, grad_out_left: torch.Tensor, grad_out_right: torch.Tensor):
        ...
        x, left_weight, left_gate_weight, right_weight, right_gate_weight = (
            x.to(op_dtype),
            left_weight.to(op_dtype),
            left_gate_weight.to(op_dtype),
            right_weight.to(op_dtype),
            right_gate_weight.to(op_dtype),
        )
        ...
        x_t = x.T
        dWLg = torch.matmul(x_t, dLA).float()
        dWL = torch.matmul(x_t, dLB).float()
        dWRg = torch.matmul(x_t, dRA).float()
        dWR = torch.matmul(x_t, dRB).float()
