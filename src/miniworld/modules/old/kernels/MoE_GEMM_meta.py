...
from miniworld.modules.old.kernels.utils import get_seq_group
...
class ContiguousGroupedGEMM(torch.autograd.Function):
    ...
    def forward(ctx, inputs, expert_weights, expert_indices, group_size_m=128):
        ...
        op_dtype = inputs.dtype

        # Save for backward
        ctx.save_for_backward(inputs.bfloat16(), expert_weights, expert_indices)
        ctx.group_size_m = group_size_m
        ctx.op_dtype = op_dtype

        return cg_grouped_gemm_forward(
            inputs=inputs,
            expert_weights=expert_weights.to(op_dtype),
