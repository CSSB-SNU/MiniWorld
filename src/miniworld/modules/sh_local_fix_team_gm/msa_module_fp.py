...
from team_gm.modules.primitives import Dropout, Linear
...
class MSAModule(torch.nn.Module):
    ...
    def __init__(self, config: Config) -> None:
        ...
        if self.use_fingerprint:
            if not config.msa_fingerprint_path:
                msg = "msa_fingerprint_path must be set when using fingerprint embedding."
                raise ValueError(msg)
            self.embed_msa = Linear(config.d_msa_fingerprint + 2, config.d_msa, bias=False)
        ...
        self.blocks.append(MSAModuleBlock(
            d_msa=config.d_msa,
            d_pair=config.d_pair,
            d_hidden_msa=config.d_hidden_msa,
            d_hidden_tri_multi=config.d_hidden_tri_multi,
