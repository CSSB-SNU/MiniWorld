...
def init_atom_features(
    reference: ReferenceFeatures | None,
    ...
    if reference is not None:
        ...
        B, L_atom = structure.atom_mask.shape
        device = structure.atom_pos.device
        dtype = structure.atom_pos.dtype

        atom_single_init = torch.zeros((B, L_atom, 6), device=device, dtype=dtype)
        atom_pair_init = torch.zeros((B, L_atom, L_atom, 5), device=device, dtype=dtype)
...
class InputAtomAttentionEncoder(nn.Module):
    ...
    def forward( # pyright: ignore[reportIncompatibleMethodOverride]
        ...
        """Forward pass."""
        atom_single_init, atom_pair_init = init_atom_features(reference, structure)
...
class InputFeatureEmbedder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if self.use_fingerprint:
            ...
            self.register_buffer("fp_table", fp_table.float())
    ...
    def forward(
        ...
        if self.use_fingerprint:
            residue_type = F.embedding(sequence.residue_type.long(), self.fp_table)
        else:
