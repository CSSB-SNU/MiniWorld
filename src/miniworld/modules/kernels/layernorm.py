...
if AUTOTUNE:
    ...
    configs = [
        triton.Config({"BLOCK_M": 32}, num_warps=16, num_stages=2),
        triton.Config({"BLOCK_M": 32}, num_warps=8, num_stages=3),
        triton.Config({"BLOCK_M": 32}, num_warps=4, num_stages=3),
        triton.Config({"BLOCK_M": 64}, num_warps=16, num_stages=2),
...
# fmt: off
@triton.autotune(configs=configs, key=["N", "GROUP_M"])
@triton.jit
...
class TritonLayerNormFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        # 20250719, for layernorm, we need to use float32
        out_dtype = x.dtype
        ...
        ctx.is_weight_none = False
        if weight is None:
            weight = torch.ones(N, dtype=op_dtype, device=x.device)
            ctx.is_weight_none = True

        ctx.is_bias_none = False
        if bias is None:
            bias = torch.zeros(N, dtype=op_dtype, device=x.device)
            ctx.is_bias_none = True
    ...
    def backward(ctx, dy: torch.Tensor):
        ...
        # allocate output
        dx = torch.empty_like(dy).to(op_dtype)
        dy = dy.to(op_dtype)
        ...
        return (
            dx.to(dtype=out_dtype),
            dw if not ctx.is_weight_none else None,
            db if not ctx.is_bias_none else None,
            None,
        )
