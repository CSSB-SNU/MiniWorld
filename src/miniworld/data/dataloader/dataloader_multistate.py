...
from miniworld.data.features.batch_multistate import (
    Batch,
    MSAFeatures,
...
from miniworld.data.mols.cifmol_attached import (
    CIFMolAttached,
    cifmol_attached_from_cifmol,
)
...
def make_batch(  # noqa: PLR0915
    cifmols: list[CIFMolAttached],
    query_cifmol: CIFMolAttached,
    msa: MSAFeatures,
) -> Batch:
    """Make batch from cifmols and msa."""
    ...
    structure = MultiStateStructureFeatures.from_sample(
    ...
    return Batch(
        name=[f"{query_cifmol.id}"],
        heteros=[query_cifmol.residues.hetero],
...
class BioMolMonomerData(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.cropper = Cropper(
            ...
            monomer_only=config.crop_config.monomer_only,
        )
    ...
    def get_item_by_seq_id(
        self,
        query_id: str,
        residue_crop_length: int | None = None,
        atom_crop_length: int | None = None,
    ) -> Batch:
        """Get item by seq_id."""
        ...
        cifmols = [
            cifmol_attached_from_cifmol(
                cifmol=cifmol,
                cluster_id_list=[cluster_id for _ in range(len(cifmol.chains))],
                seq_id_list=[query_id for _ in range(len(cifmol.chains))],
            )
            for cifmol in cifmols
        ]
        # convert to CIFMolAttached
        ...
        if residue_crop_length is None:
            residue_crop_length = self.config.crop_config.residue_crop_length
        if atom_crop_length is None:
            atom_crop_length = self.config.crop_config.atom_crop_length
        crop_indices, chain_id_to_crop_indices = self.cropper.crop(
            cifmol=query_cifmol,
            crop_length=residue_crop_length,
        )
        ...
        complex_msa = load_msa(
            ...
            env_path=self.config.preprocess_config.a3m_db_path,
        )
        msa = sample_msa(
            msa=complex_msa,
