...
def get_chain_crop_indices(
    cifmol: CIFMol, crop_indices: ndarray,
) -> dict[str, ndarray]:
    ...
    for chain in chain_list:
        ...
        chain_crop_indices = np.intersect1d(residue_indices, crop_indices) - residue_indices[0]
...
def get_valid_indices(cifmol: CIFMol) -> ndarray:
    ...
    valid_atoms_indices = np.all(~np.isnan(cifmol.atoms.xyz),axis=-1)
...
def crop_contiguous_monomer(
    ...
    polymer_chain_list = cifmol.chains[cifmol.chains.entity_type != "non-polymer"].chain_id.value
...
def crop_spatial_atom(
    ...
    pivot_atom_indices = cifmol.index_table.residues_to_atoms(np.array([pivot_residue_idx]))
    ...
    distance_map = np.linalg.norm(xyz[:, None, :] - pivot_xyz[None, :, :], axis=-1) # (N_atoms, N_pivot_atoms)
    distance_map[np.isnan(distance_map)] = float("inf")
    distance_map[~np.isin(np.arange(xyz.shape[0]), valid_atom_indices), :] = float("inf")
...
def crop_spatial_interface(  # noqa: PLR0915
    ...
    if interface_bias is not None:
        if interface_bias[0] not in valid_chain_list or interface_bias[1] not in valid_chain_list:
            msg = f"Invalid chain: {interface_bias} \
            valid_chain_list = {valid_chain_list}"
            raise ValueError(msg)
        if (interface_bias[0], interface_bias[1]) not in valid_edges and \
            (interface_bias[1], interface_bias[0]) not in valid_edges:
    ...
    distance_map[np.isnan(distance_map)] = float("inf")
    distance_map[np.arange(distance_map.shape[0]), np.arange(distance_map.shape[0])] = float("inf")

    interface_distance_map = distance_map[:len(src_atom_indices), len(src_atom_indices):] # (L_src, L_dst)
    ...
    interface_src_residues = np.where(interface_src_residues_dist <= interface_distance_cutoff)[0]
    interface_dst_residues = np.where(interface_dst_residues_dist <= interface_distance_cutoff)[0]
    ...
    rest_atom_to_res = cifmol.index_table.atoms_to_residues(np.setdiff1d(np.arange(xyz.shape[0]), interface_atoms))
    rest_order = np.argsort(rest_atom_to_res)
    rest_res_sorted = rest_atom_to_res[rest_order]
    rest_starts = np.flatnonzero(np.r_[True, rest_res_sorted[1:] != rest_res_sorted[:-1]])
    rest_min_distance_sorted = rest_min_distance[rest_order]
    rest_min_distance_per_res = np.minimum.reduceat(rest_min_distance_sorted, rest_starts)
...
def crop_spatial_interface_residue(  # noqa: PLR0915
    ...
    interface_distance_map = distance_map[:len(src_residue_indices), len(src_residue_indices):] # (L_src, L_dst)
    ...
    interface_src_residues = np.where(interface_src_distance <= interface_distance_cutoff)[0]
    interface_dst_residues = np.where(interface_dst_distance <= interface_distance_cutoff)[0]
...
class Cropper:
    ...
    def crop(
        ...
        method = random.choices(
            population=["contiguous", "spatial", "spatial_interface"],
            weights=[self.contiguous_prob,
                     self.spatial_prob,
                     self.interface_prob],
            k=1,
        )[0]
