...
def _build_atom_is_rep(
    ...
    """Select one chemical representative per token, then apply the observed mask.

    Canonical protein: CB, except GLY uses CA. Purines: C4; pyrimidines: C2
    (base atoms, not the primed sugar atoms). Missing designated atoms do not
    fall back to a different atom. Noncanonical tokens are single atoms in the
    production atom tokenizer; grouped fragments use their first atom by name.
    """
    names = np.char.strip(np.char.upper(np.asarray(cifmol.atoms.id).astype(str)))
    tok = np.asarray(atom_to_token_idx_map, dtype=np.int64)
    atom_res = np.asarray(cifmol.index_table.atom_to_res, dtype=np.int64)
    residue_names = np.char.strip(np.char.upper(
        np.asarray(cifmol.residues.chem_comp_id.value).astype(str)))
    components = residue_names[atom_res]
    valid = np.asarray(atom_pos_mask, dtype=bool)
    in_range = (tok >= 0) & (tok < n_tokens)
    canonical = np.isin(components, list(CANONICAL_CHEMCOMPS))
    desired = np.full(names.shape, 'CB', dtype='<U2')
    desired[components == 'GLY'] = 'CA'
    desired[np.isin(components, ['A', 'G', 'DA', 'DG'])] = 'C4'
    desired[np.isin(components, ['C', 'U', 'DC', 'DT'])] = 'C2'
    eligible = in_range & (~canonical | (names == desired))

    # Select before applying the observed-coordinate mask, so missing atoms do
    # not change which chemical site a token represents. Atom ordering must not
    # change a multi-atom fragment's representative either.
    candidates = np.flatnonzero(eligible)
    order = np.lexsort((candidates, names[candidates], tok[candidates]))
    candidates = candidates[order]
    _, first = np.unique(tok[candidates], return_index=True)
    chosen = candidates[first]
    atom_is_rep = np.zeros(names.shape, dtype=bool)
    atom_is_rep[chosen] = valid[chosen]
    return atom_is_rep
