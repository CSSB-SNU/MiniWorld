...
from miniworld.models.af3_natten.model import (
    AF3NattenInferenceOutput,
    AF3NattenModel,
    AF3NattenModelWrapper,
)


class AF3NattenClient(BaseClient):
    """Client for training and inference of AF3Natten model."""
    ...
    class Config(BaseModel):
        """Configuration for the AF3Natten client."""

        data: "AF3NattenClient.DataConfig"
        model: AF3NattenModel.Config
        experiment: "AF3NattenClient.ExperimentsConfig"
        diffuser: "AF3NattenClient.DiffuserConfig"
        loss: "AF3NattenClient.LossConfig"

    def __init__(self, config: Config) -> None:
        super().__init__(config)
        self.config = config
        self.set_seed(config.experiment.seed)
        self.register_model(AF3NattenModel(config.model))
    ...
    def test_inference_quality(
        self,
        batch: Batch,
        output: AF3NattenInferenceOutput,
    ) -> dict[str, float]:
    ...
    def inference(
        ...
        raw_model = cast("AF3NattenModel", raw_model)
        model_wrapper = AF3NattenModelWrapper(
            raw_model,
        )
        batch = batch.to(device=self.device)
        ...
        return AF3NattenInferenceOutput(
