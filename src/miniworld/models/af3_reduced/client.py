...
from miniworld.models.af3_reduced.model import (
    AF3ReducedInferenceOutput,
    AF3ReducedModel,
    AF3ReducedModelWrapper,
)


class AF3ReducedClient(BaseClient):
    """Client for training and inference of AF3Reduced model."""
    ...
    class Config(BaseModel):
        """Configuration for the AF3Reduced client."""

        data: "AF3ReducedClient.DataConfig"
        model: AF3ReducedModel.Config
        experiment: "AF3ReducedClient.ExperimentsConfig"
        diffuser: "AF3ReducedClient.DiffuserConfig"
        loss: "AF3ReducedClient.LossConfig"

    def __init__(self, config: Config) -> None:
        super().__init__(config)
        self.config = config
        self.set_seed(config.experiment.seed)
        self.register_model(AF3ReducedModel(config.model))
    ...
    def inference(
        ...
        raw_model = cast("AF3ReducedModel", raw_model)
        model_wrapper = AF3ReducedModelWrapper(
            raw_model,
        )
        batch = batch.to(device=self.device)
        ...
        return AF3ReducedInferenceOutput(
