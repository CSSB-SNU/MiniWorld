...
class ContactModule(nn.Module):
    ...
    def __init__(self, config: Pairformer.Config) -> None:
        super().__init__()
        self.contact_map_embedder = nn.Sequential(
            Linear(3, config.d_pair, init="default"),
            LayerNorm(config.d_pair),
            nn.ReLU(),
            Linear(config.d_pair, config.d_pair, init="default"),
        )
        self.pairformer = Pairformer(config)

    def forward(
        self,
        contact_map: Float[torch.Tensor, "B L_token L_token 3"],
        token_pair_init: Float[torch.Tensor, "B L_token L_token d_pair"],
    ) -> Float[torch.Tensor, "B L_token L_token d_pair"]:
        """Forward pass."""
        contact_map_emb = self.contact_map_embedder(contact_map)
        token_pair = token_pair_init + contact_map_emb
        token_pair, _ = self.pairformer(pair=token_pair, single=None, mask=None)
        return token_pair
...
class StructureModel(nn.Module):
    ...
    class TrunkConfig(BaseModel):
        ...
        contact_module: Pairformer.Config
    ...
    class Config(BaseModel):
        """Configuration for the Structure model."""

        shared: SharedConfig
        input_feat_embbeder: DiffusionTransformer.Config
        trunk: "StructureModel.TrunkConfig"
        diffusion: "StructureModel.DiffusionConfig"
    ...
    def __init__(self, config: Config) -> None:
        ...
        # Trunk forward
        self.contact_module = ContactModule(config.trunk.contact_module)
    ...
    def condition_forward(
        ...
        token_pair_init = self.contact_module(
            contact_map,
            token_pair_init,
        )
    ...
    def forward(
        ...
        """Forward pass of the Structure model."""
        (
            token_single_input,
            token_single_trunk,
            token_pair_trunk,
...
class StructureModelWrapper(nn.Module):
    """Wrapper for StructureModel to handle the input and output using solver."""
...
@dataclass
class StructureModelInferenceOutput:
    """Output of the StructureModel inference."""
