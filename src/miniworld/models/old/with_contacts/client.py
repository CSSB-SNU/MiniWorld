...
from miniworld.utils.structure import extract_contact_map

from .model import (
    StructureModel,
    StructureModelInferenceOutput,
    StructureModelWrapper,
)


class StructureClient(BaseClient):
    """Client for training and inference of StructureModel model."""
    ...
    class Config(BaseModel):
        """Configuration for the StructureModel client."""

        data: "StructureClient.DataConfig"
        model: StructureModel.Config
        experiment: "StructureClient.ExperimentsConfig"
        diffuser: "StructureClient.DiffuserConfig"
        loss: "StructureClient.LossConfig"

    def __init__(self, config: Config) -> None:
        super().__init__(config)
        self.config = config
        self.set_seed(config.experiment.seed)
        self.register_model(StructureModel(config.model))
    ...
    def loss_fn(
        ...
        # randomly input contact map with some probability
        batch_size, token_length = batch.scheme.residue_idx.shape[:2]
        if random.random() < self.config.experiment.contact_prob and token_length >= 2:
            input_contact_map = torch.zeros(
                (batch_size, token_length, token_length, 3),
                device=batch.device,
            ) # 0: unknown, 1: contact, 2: non-contact
            num_contacts = random.randint(1, self.config.experiment.max_contacts)
            num_non_contacts = random.randint(1, self.config.experiment.max_contacts)
            ...
            # randomly select some contacts
            contact_indices = torch.nonzero(contact_map == 1, as_tuple=False)
            non_contact_indices = torch.nonzero(contact_map == 0, as_tuple=False)
            if contact_indices.shape[0] > 0:
                selected = contact_indices[torch.randperm(contact_indices.shape[0])[:num_contacts]]
                b, i, j = selected[:, 0], selected[:, 1], selected[:, 2]
                input_contact_map[b, i, j, 1] = 1.0   # channel=1 (contact)

            if non_contact_indices.shape[0] > 0:
                selected = non_contact_indices[torch.randperm(non_contact_indices.shape[0])[:num_non_contacts]]
                b, i, j = selected[:, 0], selected[:, 1], selected[:, 2]
                input_contact_map[b, i, j, 2] = 1.0   # channel=2 (non-contact)
        else:
            input_contact_map = torch.zeros(
                (
                    batch.scheme.residue_idx.shape[0],
                    batch.scheme.residue_idx.shape[1],
                    batch.scheme.residue_idx.shape[1],
                    3,
                ),
                device=batch.device,
    ...
    def validation_step(self, batch: Batch) -> dict[str, float]:
        ...
        output = self.inference(batch, None, timesteps=self.config.experiment.eval_timesteps)
    ...
    def test_inference_quality(
        self,
        batch: Batch,
        output: StructureModelInferenceOutput,
    ) -> dict[str, float]:
    ...
    def inference(
        self,
        batch: Batch,
        contact_map: Float[torch.Tensor, "... L L"] | None,
        timesteps: int = 100,
    ) -> StructureModelInferenceOutput:
        """Inference using the diffusion solver."""
        raw_model = getattr(self.model, "module", self.model)
        raw_model = cast("StructureModel", raw_model)
        model_wrapper = StructureModelWrapper(
            raw_model,
        )
        ...
        return StructureModelInferenceOutput(
