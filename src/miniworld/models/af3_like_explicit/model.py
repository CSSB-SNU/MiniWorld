...
def _build_profile32_to_fp_index(vocab: dict[str, int], unk_key: str = "UNK") -> torch.Tensor:
    ...
    unk = int(vocab[unk_key])
    return torch.tensor([int(vocab.get(x, unk)) for x in idx32_to_chem], dtype=torch.long)
    ...
    def __init__(self, config: Config) -> None:
        ...
        if config.token_embedding.vocab_path is not None:
            with open(config.token_embedding.vocab_path, "r") as f:
                vocab = json.load(f)
            self.profile32_to_fp_index = _build_profile32_to_fp_index(vocab)
