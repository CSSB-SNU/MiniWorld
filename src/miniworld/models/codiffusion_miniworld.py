...
class ContactLogitEmbedder(nn.Module):
    """Contact Logit Embedder."""
    ...
    def forward(
        self,
        noisy_contact_prob: torch.Tensor,  # (B, L, L)
        token_mask: torch.Tensor,  # (B, L), bool
    ) -> torch.Tensor:
        """Forward pass of ContactMapEmbedder."""
        with torch.no_grad():
            noisy_contact_logit = torch.logit(
                noisy_contact_prob,
                eps=1e-6,
            )
            noisy_contact_logit = noisy_contact_logit.unsqueeze(-1)  # (B, L, L, 1)
        token_pair = self.to_pair(noisy_contact_logit)  # (B, L, L, d_pair)
