...
from miniworld.models.with_contacts import StructureClient
...
def setup_logger(client: StructureClient) -> None:
    ...
    file_handler = logging.FileHandler(
        f"logs/with_contacts/with_contacts_{now:%Y%m%d_%H%M%S}.log",
    )
...
def train(  # noqa: PLR0912, PLR0915
    ...
    if config and not resume_from_ckpt:
        ...
        cfg = StructureClient.Config.model_validate(cfg)
        client = StructureClient(cfg)
    else:
        msg = (
...
def validate(
    ...
    if not ckpt:
        ...
        raise ValueError(msg)
    client = StructureClient.from_checkpoint(ckpt)
    ...
    for ii, _batch in enumerate(valid_loader):
        ...
        output = client.inference(batch, None, timesteps=client.config.experiment.eval_timesteps)
