...
from miniworld_engine.kernels.trimul_inproj.triton import bidirectional as triton_train
...
current_back_half = triton_train._BidirBackHalfTriton
backend_calls = []
...
def whole(compiled):
    graphs, keep = {}, []
    for label in ("previous_triton", "packed_triton", "packed_h100"):
        triton_train._BidirBackHalfTriton = (
            before._BidirBackHalfTriton if label == "previous_triton" else current_back_half
        )
        backend_calls.clear()
        ...
        model = (
            BidirectionalTriangleMultiplication(
                D,
                implementation="miniworld" if label == "packed_h100" else "triton",
                p_drop=0.25,
            )
            .cuda()
            .bfloat16()
        ...
        save()
    triton_train._BidirBackHalfTriton = current_back_half
