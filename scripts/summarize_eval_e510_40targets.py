...
def oracle_value(values: list[float], direction: str) -> float:
...
def build_oracle(results: dict[str, dict[str, dict[str, float]]], common: list[str]) -> dict[str, dict[str, float]]:
        ...
        lines.append(
            ...
            + "|".join(
                [
                    ...
                    f"{row['mean_tm']:.4f}",
                    f"{row['mean_rmsd']:.3f}",
                    f"{row['mean_rmsd_lig']:.3f}",
                    f"{row['median_lddt']:.4f}",
                    f"{row['median_tm']:.4f}",
                    f"{row['median_rmsd']:.3f}",
                    f"{row['median_rmsd_lig']:.3f}",
                ]
            )
            + "|"
        )
...
def plot_violin(out_dir: Path, all_results: dict[str, dict[str, dict[str, float]]], common: list[str], labs: list[tuple[str, str, str]]) -> None:
    rng = np.random.RandomState(0)
    fig, axes = plt.subplots(2, 2, figsize=(16, 11))
    for ax, (metric, title, ylim, _) in zip(axes.flat, METRICS):
        data = [
            [all_results[label][target].get(metric, np.nan) for target in common if not np.isnan(all_results[label][target].get(metric, np.nan))]
            for label, _, _ in labs
        ]
        parts = ax.violinplot(data, showmeans=True, widths=0.8)
        for i, body in enumerate(parts["bodies"]):
            body.set_facecolor(labs[i][2])
            body.set_alpha(0.45)
    ...
    fig.savefig(out_dir / "violin_metrics_6way.png", dpi=130)
    plt.close(fig)


def plot_per_target(out_dir: Path, all_results: dict[str, dict[str, dict[str, float]]], common: list[str], labs: list[tuple[str, str, str]], tokens: dict[str, int]) -> None:
    order = sorted(common, key=lambda target: tokens.get(target, 0))
    x = np.arange(len(order))
    width = 0.82 / len(labs)
    target_labels = [short_target(target) for target in order]
    ...
    for ax, (metric, title, ylim, _) in zip(axes, METRICS):
        ...
        for j, (label, _, color) in enumerate(labs):
            ax.bar(x + (j - center) * width, [all_results[label][target].get(metric, np.nan) for target in order], width, label=label, color=color)
        ax.set_xticks(x)
    ...
    fig.savefig(out_dir / "per_target_scores_6way.png", dpi=100)
    plt.close(fig)




def plot_summary_bars(out_dir: Path, summary_rows: list[dict[str, object]], labs: list[tuple[str, str, str]]) -> None:
    ...
    fig.savefig(out_dir / "summary_metrics_6way.png", dpi=130)
    plt.close(fig)

def summarize(all_results: dict[str, dict[str, dict[str, float]]], common: list[str], labs: list[tuple[str, str, str]]) -> tuple[list[dict[str, object]], list[dict[str, object]]]:
    summary_rows: list[dict[str, object]] = []
    per_target_rows: list[dict[str, object]] = []
    for label, dirname, _ in labs:
        row: dict[str, object] = {"method": label, "dirname": dirname, "n": len(common)}
        for metric, _, _, _ in METRICS:
            values = np.array([all_results[label][target].get(metric, np.nan) for target in common], dtype=float)
        ...
        for index, target in enumerate(common, start=1):
            target_row: dict[str, object] = {"method": label, "dirname": dirname, "rank": index, "target": target}
            for metric, _, _, _ in METRICS:
                target_row[metric] = all_results[label][target].get(metric, np.nan)
...
def main() -> None:
    ...
    parser.add_argument("--base-dir", type=Path, default=Path("outputs/eval_e510_40targets"))
    parser.add_argument("--usalign", type=Path, default=Path("tools/USalign/USalign"))
    args = parser.parse_args()

    eval_module = load_eval_module()
    results = {label: eval_module.eval_dir(args.base_dir / dirname, args.usalign, True) for label, dirname, _ in METHODS}
    ...
    labs = METHODS + [ORACLE]
    tokens = target_tokens(args.base_dir / "ropeswa_af3_e510" / "sample.log")

    summary_rows, per_target_rows = summarize(all_results, common, labs)
    write_csv(
        args.base_dir / "summary_6way.csv",
        summary_rows,
        [
            "method",
            "dirname",
    ...
    write_csv(
        args.base_dir / "per_target_scores_6way.csv",
        per_target_rows,
        ["method", "dirname", "rank", "target", "lddt", "tm", "rmsd", "rmsd_lig"],
    )
    write_markdown(args.base_dir / "summary_6way.md", summary_rows)
    plot_violin(args.base_dir, all_results, common, labs)
    plot_per_target(args.base_dir, all_results, common, labs, tokens)
    plot_summary_bars(args.base_dir, summary_rows, labs)

    print(f"MEANS ({len(common)} targets)")
    print(f"{'metric':<12}" + "".join(f"{label:>16}" for label, _, _ in labs))
    for metric, _, _, _ in METRICS:
        print(f"{metric:<12}" + "".join(f"{row[f'mean_{metric}']:>16.3f}" for row in summary_rows))
    print("SAVED")
