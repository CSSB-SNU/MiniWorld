#!/usr/bin/env python3
"""Carve the protein-RNA-only subset out of the BioMolDB valid2 set.

The RNA counterpart of ``make_prt_dna_set.py``, kept as its own script.  Source
is ``cif_attached_valid_2.lmdb`` with ``metadata/valid2_edge_node.tsv``; nothing
...
Three filters, applied in this order.
  ...
  1. *Aid removal* -- every residue whose ``chem_comp_id`` is in
     ...
     same rule ``structcooker...transforms.filtering.filter_ccd`` uses.  An entry
     is never rejected for holding aids; only the aid molecules leave.

  2. *Row* filter -- an edge_node row is kept when it is an interface row
     (``cluster2 != "None"``) whose two cluster types are RNA on one side and a
     protein on the other.  Protein means any of ``cP`` Protein(L), ``cQ``
     Protein(D), ``cA`` Antibody; RNA means ``cR``.  Cluster-type letters are the
  ...
  3. *Entry* filter -- after aid removal the structure must be a genuine
     protein-RNA complex: at least one protein chain AND at least one RNA chain,
     and no DNA (``cD``) or DNA/RNA hybrid (``cN``) chain.  A sub-entry that is
     RNA-only or protein-only is dropped even though it breaks no other rule.
...
Outputs
-------
  <basedir>/cif_attached_valid_2_onlyPrtRna.lmdb
  <basedir>/metadata/valid2_edge_node_onlyPrtRna.tsv
...
Needs biomol + structcooker, so run it with StructCooker's env::
    ...
    /home/bsoohyuncd/software/StructCooker/.pixi/envs/default/bin/python \
        scripts/make_prt_rna_set.py --dry-run
    ... /bin/python scripts/make_prt_rna_set.py
"""
...
OUT_LMDB = BASEDIR / "cif_attached_valid_2_onlyPrtRna.lmdb"
OUT_TSV = BASEDIR / "metadata/valid2_edge_node_onlyPrtRna.tsv"
...
RNA_TYPE = "R"
# DNA and DNA/RNA hybrid disqualify a sub-entry -- the mirror of the DNA set,
# which rejects cR and cN.  cN is excluded either way: it is cleanly neither.
FORBIDDEN_TYPES = frozenset("DN")
...
def is_protein_rna_pair(cluster1: str, cluster2: str) -> bool:
    """True when the two cluster ids are an RNA chain against a protein chain."""
    pair = {cluster1[1], cluster2[1]}
    return RNA_TYPE in pair and bool(pair & PROTEIN_TYPES)
...
def read_candidate_rows(tsv_path: Path) -> tuple[dict[str, list[tuple[str, ...]]], Counter]:
    """Group protein-RNA interface rows by lowercase pdb_id."""
    ...
    with tsv_path.open() as fh:
        ...
        for line in fh:
            ...
            if fields[1] == "None":
                ...
                continue
            if not is_protein_rna_pair(fields[0], fields[1]):
                continue
...
def build_entry(
    ...
    Tallies go into a local counter and are merged into ``counts`` only when the
    entry actually reaches the output, so the reported totals describe what was
    written rather than work done on entries that were later discarded.
    """
    ...
    for asm_key, item in entry.items():
        ...
        if n_aid:
            modified = True
            local["aid_residues_removed"] += n_aid
        if stripped is None:
            local["subentry_all_aid"] += 1
            continue
        if n_aid:
            local["aid_chains_removed"] += n_chains_before - len(stripped.chains)

        verdict, _ = classify(stripped)
        local[f"subentry_{verdict}"] += 1
        if verdict != "keep":
            modified = True
            continue
...
def process(*, dry_run: bool) -> None:  # noqa: PLR0915
    """Filter valid2 and, unless dry-running, write its LMDB + TSV."""
    print(f"{'=' * 72}\nvalid2 -> protein-RNA\n{'=' * 72}")
    print(f"source lmdb : {IN_LMDB}")
    print(f"source tsv  : {IN_TSV}")
    print(f"out lmdb    : {OUT_LMDB}")
    print(f"out tsv     : {OUT_TSV}")
    print(f"aid codes   : {' '.join(AID_CODES)}")

    rows_by_pdb, tally = read_candidate_rows(IN_TSV)
    print(f"\nrows in tsv                     : {tally['rows_total']}")
    print(f"  monomer rows (cluster2 None)  : {tally['rows_monomer']}")
    print(f"protein-RNA interface rows      : {tally['rows_candidate']}")
    ...
    env = lmdb.open(str(IN_LMDB), readonly=True, lock=False, max_readers=2048)
    ...
    print(f"sub-entries dropped, DNA/hybrid   : {counts['subentry_has_dna']}")
    print(f"sub-entries dropped, no protein   : {counts['subentry_no_protein']}")
    print(f"sub-entries dropped, no RNA       : {counts['subentry_no_rna']}")
    ...
    write_tsv(OUT_TSV, kept_rows)
    write_lmdb(OUT_LMDB, out_values, payload)
