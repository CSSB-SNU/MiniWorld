"""Training script for MiniWorld phase 2.

Attach an EDM diffusion module on top of the FROZEN phase 1b mini-SWA trunk and
...
Usage:
    torchrun --nproc_per_node=1 scripts/run_miniworld_diffusion_train.py train \
...
@click.option(
    "--ckpt",
    type=click.Path(exists=True, dir_okay=False, path_type=Path),
    help="phase 1b trunk checkpoint (epoch=0900.pt) to load + freeze",
)
...
@click.option(
    ...
    default=False,
    show_default=True,
    help="Exact model/optimizer match. phase 2 loads only trunk keys -> use --no-ckpt-strict.",
)
...
def train(  # noqa: PLR0912, PLR0915
    ...
    if state_dict is not None:
        if policy_summary is None:
            ...
            # restart (the schedule is relative to the phase 2 optimizer, which starts at step 0
            # when phase 2 begins). Skipped on the epoch-900 SEED, whose optimizer is over the
