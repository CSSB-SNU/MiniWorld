"""Validation-only sweep without EMA parameters for atom_token variant.

Based on scripts/run_af3like_rev.py (DefaultClient_rev + af3_like.Model_rev).
For each checkpoint in the given --ckpt-dir(s), loads only the non-EMA
model weights (uses config.train.use_ema=False so the ModelEMA callback
is not attached and the saved EMA parameters are ignored), runs one
validation pass, and appends a JSON record (epoch -> metrics) to --output.
"""
