...
# 8-way: same 6 variants as before + nosingle + ropeglobal-af3 pytorch_transition
METHODS = [
...
def main() -> None:
    ...
    labs = METHODS + [ORACLE]
    tokens = target_tokens(args.base_dir / "nosingle_e510" / "driver.log")

    summary_rows, per_target_rows = summarize(all_results, common, labs)
    write_csv(
        args.base_dir / "summary_8way.csv",
        summary_rows,
        [
            "method",
            "dirname",
    ...
    write_csv(
        args.base_dir / "per_target_scores_8way.csv",
        per_target_rows,
        ["method", "dirname", "rank", "target", "lddt", "tm", "rmsd", "rmsd_lig"],
    )
    write_markdown(args.base_dir / "summary_8way.md", summary_rows)
