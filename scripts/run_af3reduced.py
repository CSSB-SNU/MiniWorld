...
from miniworld.models.af3_reduced import AF3ReducedClient
...
def setup_logger(client: AF3ReducedClient) -> None:
    ...
    file_handler = logging.FileHandler(
        f"logs/af3reduced/af3reduced_{now:%Y%m%d_%H%M%S}.log",
    )
...
def train(  # noqa: PLR0912, PLR0915
    ...
    if config and not resume_from_ckpt:
        ...
        cfg = AF3ReducedClient.Config.model_validate(cfg)
        client = AF3ReducedClient(cfg)
    else:
        msg = (
...
def sample(
    ...
    if not ckpt:
        ...
        raise ValueError(msg)
    client = AF3ReducedClient.from_checkpoint(ckpt)
